---
description: 포스트 주제에 맞는 Unsplash 히어로 이미지 후보 추천 + 다운로드 명령어 안내
argument-hint: <포스트 경로 또는 주제 키워드>
---

# Select Hero Image from Unsplash
...
이 스크립트는 이미지를 WebP로 변환하고, 모든 언어 버전의 front matter 에서 `.jpg` → `.webp` 로 경로를 자동 업데이트합니다.
