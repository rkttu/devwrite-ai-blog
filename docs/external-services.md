...
| Kakao AdFit | [AdFit Web SDK](https://github.com/adfit/adfit-web-sdk) 광고 스크립트 | 운영 환경의 공개 포스트 | `params.integrations.kakaoadfit`, `params.kakaoadfit` |
| Utterances | [Utterances](https://utteranc.es/) 댓글 스크립트 | 운영 환경에서 댓글을 활성화한 페이지 | `params.integrations.utterances`, `params.utterances`, 페이지 `comments` |
...
1. 개발 빌드에서 Google Analytics, Kakao AdFit, Utterances 스크립트가 출력되지 않는지 검사합니다.
