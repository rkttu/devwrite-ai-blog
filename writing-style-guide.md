...
이 블로그의 글들은 AI의 도움을 받아 작성됩니다. 사이트는 이 사실을 공통 바닥글에 자동으로 표시합니다.

### 자동 표시 (시스템 레벨)

- 모든 페이지의 공통 바닥글에 AI 어시스트 안내 문구가 자동으로 표시됨
...
### 강조 표기 (Bold/Italic) 후 공백 규칙

- 닫는 `**`, `*`, `__`, `_` 마커 바로 뒤에 글자(letter)가 오는 경우 **반드시 공백 1개를 삽입**
- 일부 마크다운 렌더러가 닫는 마커 뒤에 공백 없이 글자가 붙으면 강조를 올바르게 처리하지 못하는 알려진 호환성 문제가 있음
- 닫는 마커 뒤에 구두점(`,`, `.`, `)`, `!`, `?`, `:`, `;` 등)이 오는 경우는 공백 불필요

**올바른 예시:**

- `**Runtime Async** 는 이 전체 구조를 뒤집습니다.`
- `**컴파일러(Roslyn)** 가 처리합니다.`
- `**NuGet 패키지** 없이 BCL만으로`
