...
대상 slug와 세 언어 Page Bundle을 확인합니다. `draft: false`, 사용자가 지정한 공개 시각, 번역 일관성, 이미지 존재 여부를 점검합니다. 예약 발행은 미래 날짜를 허용하며 프로덕션 빌드에서 미래 글을 제외하는지 확인합니다. 작업 트리의 사용자 변경을 모두 확인하고 대상 글과 무관한 변경을 커밋에 포함하지 않습니다.
...
공개 시각이 지난 글은 각 URL에서 HTTP 200, 최종 URL, 예상 제목, canonical URL을 확인합니다. 예약 시각 전인 글은 운영 페이지와 목록, 피드, 검색, 사이트맵에서 제외됐는지 확인하고 로컬 `--clock` 빌드로 공개 시각 이후의 산출물을 검증합니다. 예약 워크플로가 기본 브랜치에서 활성 상태인지 확인하고 예약 등록 완료와 실제 공개 완료를 구분해 보고합니다. 한국어 페이지에서는 `og:title`, `og:description`, `og:image`, `og:url`도 확인하고 대표 이미지 URL이 HTTP 200을 반환하는지 검사합니다.

CDN 반영이 늦으면 몇 분 동안 제한적으로 재시도합니다. Actions 성공만으로 운영 발행을 완료 처리하지 않습니다. 현재 시각에 공개할 운영 HTML이 예상 커밋의 내용을 보여 줄 때 배포 완료로 기록합니다. 미래 글은 예약 설정 완료로 기록합니다.
