...
제목과 본문을 작성하기 전에 `writing-style-guide.md` 전체를 읽습니다. 이 스킬의 한국어 제목과 본문은 커뮤니티 글이라는 이유로 친근한 구어체를 선택하지 않고 격식 문어체인 `~니다`체로 통일합니다. `~해요`, `~죠`, `~더라고요` 같은 종결은 사용하지 않습니다. 사용자가 이 게시물에 적용할 다른 문체를 명시한 경우에만 그 지시를 우선합니다.

소개 글에는 가이드의 1등급과 2등급 규칙을 적용합니다. 포럼 제목에는 글 제목 규칙을 적용하고 목록을 넣었다면 나열형 목록 규칙도 적용합니다. 전문을 교차 게시할 때에만 완결된 배포 문서 규칙을 적용합니다. 짧은 소개 글에 문서 골격을 억지로 추가하지 않습니다.
...
사용자에게 초안을 보여 주기 전에 가이드의 탈고 체크리스트를 적용합니다. 종결 어미 혼용, 무주체 피동, 명사화 계사문, 당위 지시, 반복한 접속 부사, 미확인 정보의 단정이 남아 있으면 먼저 고칩니다. 미리 보기 제품을 소개할 때에는 확인 기준일과 잠정성을 본문에서 함께 밝힙니다.
...
외부 상태를 바꾸기 직전에 제목, 카테고리, 추가 태그, 본문 전체, 원문 URL을 보여 주고 사용자의 최종 승인을 받습니다. disc가 `disc` 태그와 도구 출처 문구를 자동으로 추가한다는 점도 알립니다. 브라우저 작성기가 열리면 출처 문구 앞에 자동으로 붙은 이모지를 제거하고 텍스트 출처는 유지하도록 안내합니다.
