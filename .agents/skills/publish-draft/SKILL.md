...
`$review-draft` 결과에서 발행을 막는 항목이 없고 사용자가 상태 전환을 요청했을 때만 세 파일의 `draft` 값을 `false`로 바꿉니다. 사용자가 예약 발행을 요청한 경우 미래 날짜를 허용하며 세 언어의 공개 시각을 일치시킵니다.
...
`draft: false`인 글은 공개 시각이 지난 뒤 실행한 빌드부터 포함합니다. 이 작업만으로 운영 사이트에 발행되지 않습니다. 커밋과 푸시, GitHub Pages 배포는 `$deploy-blog-post`의 별도 범위입니다.
