...
공개 모델의 사양과 공식 측정값, 제가 사용한 실험 환경을 항목별로 구분했습니다.

- **공개 모델**: `Edge0-35B-A3B-preview`는 35B급 MoE 구조에 4비트 양자화를 적용합니다.

- **모델 파일 크기**: SSD에 저장하는 체크포인트가 약 23GB를 차지합니다.

- **공식 메모리 측정**: 짧은 문맥에서 MLX 할당기의 최대 활성 메모리가 약 2.9GiB를 기록했습니다.

- **개인 실험 장비**: 이 연재에서는 M2 MacBook Air 16GB를 사용했습니다.

- **첫 생성 실험**: 특정 입력과 실행 조건에서 128토큰을 생성하는 데 약 43.8초가 걸렸습니다.
...
- **그라운딩 자료 약 6.7KB**: 전체 처리에 약 94초가 걸렸으며 이때의 답변을 비교 기준으로 사용했습니다.

- **그라운딩 자료 약 3.4KB**: 전체 처리에 약 78초가 걸렸습니다. 자료를 줄인 뒤의 답변을 비교했습니다.

- **그라운딩 자료 약 1.7KB**: 전체 처리에 약 48초가 걸렸으며 핵심 답변도 유지했습니다.
