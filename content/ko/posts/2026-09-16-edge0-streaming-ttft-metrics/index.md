...
- **TTFT**: 요청 기준 시각부터 첫 콘텐츠를 수신하기까지 걸린 시간을 잽니다. 클라이언트에서 관찰한 첫 출력 대기 시간을 나타냅니다.

- **총 생성 시간**: 요청 기준 시각부터 스트림 수신이 끝날 때까지 걸린 시간을 잽니다. 검색 시간은 포함하지 않으며 한국어 번역이 모두 끝나는 시점까지 측정하지 않습니다.

- **생성량**: 콘텐츠가 있는 SSE 이벤트 수를 셉니다. 현재 전송 동작을 전제로 이 값을 토큰 수의 추정치로 사용합니다.

- **첫 응답 이후 시간**: 첫 콘텐츠를 받은 시점부터 스트림 수신이 끝날 때까지 걸린 시간을 잽니다. 네트워크 수신과 종료 처리에 걸리는 시간도 포함합니다.

- **출력 속도**: 생성량을 첫 응답 이후 시간으로 나눕니다. 콘텐츠 이벤트를 기준으로 처리율을 계산합니다.
