...
| 指標 | 計算基準 | 解釈の範囲 |
| --- | --- | --- |
| TTFT | リクエストの基準時刻から最初のコンテンツ受信まで | クライアントが観測した最初の出力までの待ち時間 |
| 総生成時間 | リクエストの基準時刻から受信終了まで | 検索と韓国語翻訳全体の完了時間は含まない |
| 生成量 | コンテンツを含むSSEイベント数 | 現在の転送動作を前提にしたトークン数の推定 |
| 最初の応答後の時間 | 最初のコンテンツ受信から受信終了まで | ネットワーク受信や終了処理も含む時間 |
| 出力速度 | 生成量を最初の応答後の時間で割った値 | コンテンツイベント単位の処理率 |
